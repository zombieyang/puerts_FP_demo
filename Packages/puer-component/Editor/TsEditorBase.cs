    ...
    public class TsEditorBase : UnityEditor.Editor
    {
        ...
        protected static JsEnv EditorJsEnv
        {
            get
            {
                ...
                {
                    var jsEnv = new JsEnv(JsEnvLoader == null ? new TSLoader() : JsEnvLoader);
        ...
        private Rect RenderPrimitiveProp(IPrimitivePropertySerializer serializer, string name, SerializedProperty prop,
            Type type, int indent, Property property)
        {
                ...
                for (int i = 0; i < prop.arraySize; i++)
                {
                    ...
                    if (Event.current.type == EventType.ContextClick && rect1.Contains(Event.current.mousePosition))
                    {
                        ...
                        menu.AddItem(new GUIContent("Delete Element"), false,
                            (userData) => { toDeleteElementPath = (string)userData; }, elementProp.propertyPath);
                        menu.ShowAsContext();
                        Event.current.Use();
                    }
